...
class Interaction(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.mlp = FullyConnectedNet(
            radial_neurons + [self.conv.weight_numel], torch.nn.functional.silu)
    ...
    def forward(self, x, node_attr, edge_index, edge_attr, edge_len_emb):
        ...
        edge_features = self.conv(
            node_features[i], edge_attr, weight=self.mlp(edge_len_emb))
        node_features = scatter(edge_features, j, dim=0, dim_size=num_nodes).div(
